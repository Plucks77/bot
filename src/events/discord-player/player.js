...
const registerPlayerEvents = (player, client) => {
  player.events.on("connection", async (queue) => {
    ...
    queue.dashboard = await queue.metadata.channel.send({
      embeds: [embed.Embed_connect()],
      components: [row],
    });
  });
    ...
    if (error.message.includes("Sign in to confirm your age")) {
      if (queue.tracks.data.length < 1) {
        ...
        return queue.metadata.channel.send({
          content: "❌ | I can't play Age-restricted videos.",
      ...
      queue.node.skip();
      return queue.metadata.channel.send({
        content: "❌ | I skipped Age-restricted video.",
        allowedMentions: { repliedUser: false },
      });
    }
  });
