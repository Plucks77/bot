...
module.exports = async (client, int) => {
  if (int.isButton()) {
    if (
      blockedUsers.includes(int.user.username) &&
      blockedCommands.includes(int.customId)
    ) {
      return int.reply({
        content: meMama,
    ...
    if (
      int.guild.members.me.voice.channel &&
      int.member.voice.channelId !== int.guild.members.me.voice.channelId
    ) {
      return int.reply({
    ...
    const description = `Author : **${track.author}**\nDuration **${trackDuration}**`;
      ...
      case "Save Song":
        {
          int.member
            ...
            .then(() =>
              int.reply({
                content:
                  "✅ | I sent you the name of the music in a private message.",
                ephemeral: true,
                components: [],
              })
            )
      ...
      case "Playing-PlayPause":
        {
          ...
          const playPauseButton = new ButtonBuilder()
            .setCustomId("Playing-PlayPause")
            .setEmoji(playing ? button.play : button.pause)
            .setStyle(playing ? ButtonStyle.Success : ButtonStyle.Secondary);
          const skipButton = new ButtonBuilder()
            .setCustomId("Playing-Skip")
            .setEmoji(button.skip)
            .setStyle(ButtonStyle.Secondary);
          const stopButton = new ButtonBuilder()
            .setCustomId("Playing-Stop")
            .setEmoji(button.stop)
            .setStyle(ButtonStyle.Danger);
          const loopButton = new ButtonBuilder()
            .setCustomId("Playing-Loop")
            .setEmoji(button.loop)
            .setStyle(ButtonStyle.Secondary);
          const shuffleButton = new ButtonBuilder()
            .setCustomId("Playing-Shuffle")
            .setEmoji(button.shuffle)
            .setStyle(ButtonStyle.Secondary);
      ...
      case "Playing-Loop":
        {
          ...
          const msg = await int.reply({
            content: "Select a song loop mode.",
            ephemeral: true,
            components: [row],
          });
          ...
          collector.on("collect", async (i) => {
            ...
            await i.deferUpdate();
            await int.editReply({
              content: `✅ | Set loop to \`${methods[mode]}\`.`,
              ephemeral: true,
              components: [],
            });
            ...
            return await queue.dashboard.edit({
              embeds: [
                embed.Embed_dashboard(
                  "Dashboard",
                  track.title,
                  track.url,
                  track.thumbnail,
    ...
    if (cmd) {
      console.log(
        `(${color.grey}${int.member.guild.name}${color.white}) ${int.user.username} : /${int.commandName}`
      );

      if (
        blockedUsers.includes(int.user.username) &&
        blockedCommands.includes(int.commandName)
      ) {
        return int.reply({
          content: meMama,
          allowedMentions: { repliedUser: false },
