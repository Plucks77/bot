module.exports = {
  ...
  async execute(client, message, args) {
    ...
    if (queue.volume === vol) {
      return message.reply({
        content: '❌ | The volume you want to change is already the current volume.',
        allowedMentions: { repliedUser: false },
      });
    }
    ...
    if (vol < 0 || vol > maxVolume) {
      return message.reply({
        content: `❌ | **Type a number from \`1\` to \`${maxVolume}\` to change the volume .**`,
        allowedMentions: { repliedUser: false },
      });
    }

    const success = queue.node.setVolume(vol);
    const replymsg = success
      ? `🔊 **${vol}**/**${maxVolume}**%`
