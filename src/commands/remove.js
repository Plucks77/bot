...
module.exports = {
  ...
  async execute(client, message) {
    ...
    if (!queue || !queue.isPlaying()) return message.reply({ content: '❌ | There is no music currently playing.', allowedMentions: { repliedUser: false } });

    const tracks = queue.tracks.map((track, index) => `${++index}. ${track.title}`);

    if (tracks.length < 1) return message.reply({ content: '❌ | No music in queue after current.', allowedMentions: { repliedUser: false } });
    ...
    const loopStatus = queue.repeatMode ? (queue.repeatMode === 2 ? 'All' : 'ONE') : 'Off';
    ...
    collector.on('end', (msg, reason) => {
      if (reason === 'time') return message.reply({ content: '❌ | Song remove time expired', allowedMentions: { repliedUser: false } });
    });
  },
  ...
  async slashExecute(client, interaction) {
    ...
    if (!queue || !queue.isPlaying()) return interaction.reply({ content: '❌ | There is no music currently playing.', allowedMentions: { repliedUser: false } });

    const tracks = queue.tracks.map((track, index) => `${++index}. ${track.title}`);

    if (tracks.length < 1) return interaction.reply({ content: '❌ | No music in queue after current.', allowedMentions: { repliedUser: false } });
    ...
    collector.on('end', (msg, reason) => {
      if (reason === 'time') return interaction.reply({ content: '❌ | Song remove time expired', allowedMentions: { repliedUser: false } });
    });
  },
};
