...
module.exports = {
  ...
  async execute(client, message, args) {
    if (!args[0]) return message.reply({ content: '❌ | Please enter a valid song name.', allowedMentions: { repliedUser: false } });
    ...
    if (!results || !results.hasTracks()) return message.reply({ content: '❌ | No results found.', allowedMentions: { repliedUser: false } });
  ...
  async slashExecute(client, interaction) {
    ...
    if (!results || !results.hasTracks()) return interaction.editReply({ content: '❌ | No search results found.', allowedMentions: { repliedUser: false } });
    ...
    const row = new ActionRowBuilder().addComponents(select);
    const msg = await interaction.editReply({ components: [row] });
