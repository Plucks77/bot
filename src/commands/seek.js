...
module.exports = {
  ...
  execute(client, message, args) {
    ...
    if (!args[0]) return message.reply({ content: '❌ | Write the time of the music you want to seek.', allowedMentions: { repliedUser: false } });

    const track = queue.currentTrack;
    const timestamp = queue.node.getTimestamp();
    if (timestamp.progress == 'Forever') return message.reply({ content: '❌ | Can\'t seek in a live stream.', allowedMentions: { repliedUser: false } });

    const str = args.join(' ');
    const tragetTime = timeToSeconds(str);
    const musicLength = timeToSeconds(track.duration);

    if (!tragetTime) return message.reply({ content: '❌ | Invalid format for the target time.\n(**`ex: 3m20s, 1m 50s, 1:20:55, 5:20`**)', allowedMentions: { repliedUser: false } });

    if (tragetTime >= musicLength) return message.reply({ content: `❌ | Target time exceeds music duration. (\`${track.duration}\`)`, allowedMentions: { repliedUser: false } });
  ...
  slashExecute(client, interaction) {
    ...
    const track = queue.currentTrack;
    const timestamp = queue.node.getTimestamp();
    if (timestamp.progress == 'Forever') return interaction.reply({ content: '❌ | Can\'t seek in a live stream.', allowedMentions: { repliedUser: false } });
    ...
    if (tragetTime >= musicLength) return interaction.reply({ content: `❌ | Target time exceeds music duration. (\`${track.duration}\`)`, allowedMentions: { repliedUser: false } });
